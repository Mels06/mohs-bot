...
const SCRIPT_URL      = process.env.SCRIPT_URL      || "https://script.google.com/macros/s/AKfycbxZSRdOawnvz-gl29DATsg_BC4W8kmvqYFbLbaSCked2SzH_369l5gpcEbjsLdrE6jm/exec";
...
async function envoyerMail({ email, nom, id, pack, montant, plateforme, date_fin, lienPaiement }) {
  ...
  <table width="600" style="margin:auto;background:#fff;border-radius:12px;overflow:hidden;">
    ...
    <tr><td style="padding:30px;">
      <h2>Bienvenue, ${nom} !</h2>
      <p>Votre abonnement a ete enregistre. Voici votre recapitulatif :</p>
      <table width="100%" style="border:1px solid #eee;border-radius:8px;overflow:hidden;">
        ...
        <tr><td style="padding:10px 15px;color:#888;border-bottom:1px solid #eee;">ID Client</td><td style="padding:10px 15px;border-bottom:1px solid #eee;"><span style="background:#1a1a2e;color:#F5A623;padding:3px 10px;border-radius:15px;font-family:monospace;">${id}</span></td></tr>
        <tr style="background:#f9f9f9;"><td style="padding:10px 15px;color:#888;border-bottom:1px solid #eee;">Pack</td><td style="padding:10px 15px;font-weight:bold;border-bottom:1px solid #eee;">${pack}</td></tr>
        <tr><td style="padding:10px 15px;color:#888;border-bottom:1px solid #eee;">Plateforme</td><td style="padding:10px 15px;border-bottom:1px solid #eee;">${plateforme}</td></tr>
        <tr style="background:#f9f9f9;"><td style="padding:10px 15px;color:#888;border-bottom:1px solid #eee;">Montant/mois</td><td style="padding:10px 15px;color:#F5A623;font-weight:bold;border-bottom:1px solid #eee;">${Number(montant).toLocaleString("fr-FR")} FCFA</td></tr>
        <tr><td style="padding:10px 15px;color:#888;">Valide jusqu'au</td><td style="padding:10px 15px;font-weight:bold;">${date_fin}</td></tr>
      </table>
      ${lienHtml}
      <p>Conservez votre ID <strong>${id}</strong> pour tout support.</p>
      <p>Contact : <a href="mailto:contact@mohstechnologie.com" style="color:#F5A623;">contact@mohstechnologie.com</a></p>
    </td></tr>
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: "MOHS TECHNOLOGIE <onboarding@resend.dev>", to: [email], subject: "Bienvenue chez MOHS TECHNOLOGIE - " + pack, html })
    });
...
function menuAdmin() {
  return "MOHS BOT - Panneau Admin\n\n" +
    ...
    "nouveau [nom] [tel] [email] [pack1/2/3/4] [telegram/whatsapp] [nb_mois]\n" +
    ...
    "LISTES FILTREES :\n" +
    "liste pack1 / pack2 / pack3 / pack4\n" +
    "liste telegram / whatsapp\n" +
    "liste actifs / expires\n\n" +
    "ABONNEMENTS :\n" +
    "renouveler [ID] - 1 mois\n" +
    "renouveler [ID] [nb mois] - ex: renouveler MT-X7K2 3\n" +
    "suspendre [ID]\n" +
    "reactiver [ID]\n\n" +
    "FINANCES :\n" +
    "ca [date1] [date2] - ex: ca 01/01/2025 31/03/2025\n" +
    "ca [mois] - ex: ca mars\n" +
    "ca [annee] - ex: ca 2025\n\n" +
    "COMMANDES :\n" +
  ...
  try {
    ...
    if (text.toLowerCase().startsWith("nouveau ")) {
      ...
      const mailEnvoye = await envoyerMail({
        ...
        plateforme: plateforme.charAt(0).toUpperCase() + plateforme.slice(1),
        date_fin: result.date_fin, lienPaiement: null
      });
      ...
      msg += "Montant total : " + (montant * nbMois).toLocaleString("fr-FR") + " FCFA\n";
      msg += "Valide jusqu'au : " + result.date_fin + "\n\n";
      msg += mailEnvoye ? "Mail de bienvenue envoye a " + email + " ✅" : "Mail non envoye - domaine non verifie sur Resend";
      await send(chatId, msg);
      return;
    }
    ...
    // LISTE FILTREE : liste pack1 / liste telegram / liste actifs etc.
    if (text.toLowerCase().startsWith("liste ")) {
      ...
      msg += "\nTotal : " + result.total + " client(s)";
      await send(chatId, msg);
      return;
    }

    // RENOUVELER [ID] ou [ID] [nb mois]
    ...
    if (text.toLowerCase().startsWith("ca ")) {
      ...
      if (args.length === 1 && isNaN(args[0]) && MOIS[args[0].toLowerCase()]) {
        const m   = MOIS[args[0].toLowerCase()];
        const y   = new Date().getFullYear();
        const pad = n => String(n).padStart(2,"0");
        const dernierJour = new Date(y, m, 0).getDate();
        debut = pad(1) + "/" + pad(m) + "/" + y;
        fin   = pad(dernierJour) + "/" + pad(m) + "/" + y;
      ...
      for (const [pack, v] of Object.entries(result.par_pack || {})) {
        msg += "  " + pack + " : " + v.nb + " paiement(s) - " + Number(v.montant).toLocaleString("fr-FR") + " FCFA\n";
      }
      await send(chatId, msg);
      return;
    }
    ...
    if (text.toLowerCase().startsWith("commandes")) {
      ...
      if (result.status !== "ok" || result.total === 0) {
        await send(chatId, "Aucune commande trouvee.");
        return;
      }
      let msg = aujourdhui ? "Commandes du jour (" + result.total + ")\n\n" : "Toutes les commandes (" + result.total + ")\n\n";
    ...
    if (text.toLowerCase() === "stats") {
      ...
      msg += "\nPar plateforme :\n";
      msg += "  Telegram : " + (result.par_plateforme?.telegram || 0) + " client(s)\n";
      msg += "  WhatsApp : " + (result.par_plateforme?.whatsapp || 0) + " client(s)";
      await send(chatId, msg);
      return;
    }
...
app.post("/paiement-confirme", async (req, res) => {
  ...
  } catch(e) { console.error("FedaPay:", e.message); }
});
