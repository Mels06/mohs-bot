...
// Réponses conversationnelles sur MOHS TECHNOLOGIE
const CONVERSATIONS = {
  salutations: ["bonjour","bonsoir","salut","hello","hi","coucou","yo","hey"],
  quiSommes: ["qui es tu","qui etes vous","c'est quoi mohs","keskesvous","que faites vous","presentez vous","presentation","vous faites quoi","tu fais quoi"],
  services: ["services","offres","produits","bots","bot","pack","packs","prix","tarif","tarifs","combien"],
  contact: ["contact","email","telephone","joindre","appeler","ecrire","whatsapp"],
...
function repondreConversation(intent, prenom) {
  ...
  switch(intent) {
    case "salutations":
      return "Bonjour" + n + " ! Je suis ton assistant MOHS TECHNOLOGIE. Comment puis-je t'aider ?\n\nTape 'aide' pour voir toutes les commandes disponibles.";
    case "quiSommes":
      return "MOHS TECHNOLOGIE est une structure specialisee dans la configuration de bots intelligents pour les entreprises et particuliers.\n\nNos bots peuvent gerer :\n- Les ventes et commandes\n- Le service client\n- La gestion commerciale\n- Les assistants automatises\n\nNous proposons 4 packs adaptes a tous les besoins. Tape 'packs' pour les voir !";
    case "services":
      return "Nous proposons 4 packs de bots intelligents :\n\nPack 1 - Essentiel\nPack 2 - Avancee\nPack 3 - Assistant\nPack 4 - Commercial\n\nDisponibles sur Telegram et WhatsApp.\nTape 'packs' pour voir les details et tarifs !";
    case "contact":
      return "Pour nous contacter :\n\nEmail : contact@mohstechnologie.com\n\nNous repondons dans les plus brefs delais !";
    case "merci":
      return "Avec plaisir" + n + " ! N'hesite pas si tu as d'autres questions.";
    case "aide":
      return "Voici les principales commandes :\n\n" +
        "nouveau [nom] [tel] [email] [pack] [plateforme] - Enregistrer un client\n" +
        ...
        "renouveler [ID] [nb mois] - Renouveler plusieurs mois\n" +
        "stats - Tableau de bord\n" +
        "ca mars / ca 2025 - Chiffre d'affaires\n" +
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    if (text.toLowerCase().startsWith("nouveau ")) {
      ...
      let msg = "Client enregistre avec succes !\n\n";
      msg += "ID : " + idClient + "\n";
      msg += "Nom : " + nom + "\n";
      msg += "Tel : " + telephone + "\n";
      msg += "Email : " + email + "\n";
      ...
      msg += mailEnvoye ? "Mail de bienvenue envoye a " + email + " ✅" : "Mail non envoye - verifier configuration Resend";
      await send(chatId, msg);
      return;
    }
    ...
    if (text.toLowerCase().startsWith("ca ")) {
      ...
      if (args.length === 1 && isNaN(args[0]) && MOIS[args[0].toLowerCase()]) {
        ...
        debut = pad(1) + "/" + pad(m) + "/" + y;
        fin   = pad(new Date(y, m, 0).getDate()) + "/" + pad(m) + "/" + y;
    ...
    if (text.toLowerCase() === "stats") {
      ...
      msg += "\nPar plateforme :\n  Telegram : " + (result.par_plateforme?.telegram || 0) + " client(s)\n  WhatsApp : " + (result.par_plateforme?.whatsapp || 0) + " client(s)";
      await send(chatId, msg);
      return;
    }
    ...
    if (intent) {
      const reponse = repondreConversation(intent, prenom);
